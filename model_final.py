...
def simulation(settings_selection):
    ...
    for row_index in df_results.iterrows():
        # Nur zu Öffnungszeiten Zugänglich?
        if not int(settings["accessible_all_day"]):
            # Prüfe, ob innerhalb der Öffnungszeiten, erst dann werden neue cars erzeugt:
            if ((settings["opening_time_in_min"] <= minute_counter) and
                    (settings["closing_time_in_min"] > minute_counter)):
        ...
        for _ in range(num_new_cars):
            ...
            total_parking_duration = int(stats.exponweib.rvs(settings["a_out"], settings["kappa_out"], \
                                                         loc=settings["loc_out"], scale=settings["lambda_out"], size=1)) # Parkdauer mit Weibull
...
def main():
    ...
    df_results_returned, anzahl_evs, anzahl_evs_abgewiesen = simulation("settings_final.json")
